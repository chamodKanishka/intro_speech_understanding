...
def dft_matrix(N):
    ...
    '''
    W = np.zeros((N, N), dtype='complex')  # Initialize a matrix of zeros with complex dtype
    for k in range(N):
        for n in range(N):
            W[k, n] = np.cos(2 * np.pi * k * n / N) - 1j * np.sin(2 * np.pi * k * n / N)  # Compute each element
    return W  # Return the result